...
Currently configured project: `port-swap` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/port-swap/SETUP.md` to fill in
   `data/port-swap/INFO.md`.
...
`--project-id port-swap` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/port-swap/`.
