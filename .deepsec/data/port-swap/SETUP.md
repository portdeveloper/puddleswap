# Agent setup for `port-swap`

This is a deepsec scanning workspace. Project `port-swap` was just registered
(target: `..`). Setup is incomplete — `data/port-swap/INFO.md`
...
2. **Fill in `data/port-swap/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id port-swap
pnpm deepsec process --project-id port-swap
```
