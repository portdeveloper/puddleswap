...
MON_PER_POOL="5000000000000000000000"   # 5000e18
USDC_AMOUNT="5000000000000"              # 5,000,000 USDC (6 decimals)
USDT_AMOUNT="5000000000000"              # 5,000,000 USDT (6 decimals)
DEADLINE=$(($(date +%s) + 3600))

echo "=== Step 1: Skipped (real USDC — wallet must be pre-funded) ==="

echo ""
echo "=== Step 2: Admin-mint 5,000,000 USDT ==="
cast send $FAUCET "adminMint(address,address,uint256)" \
  $USDT $ADDR $USDT_AMOUNT \
  --account $ACCOUNT --rpc-url $RPC

echo ""
echo "=== Step 3: Wrap 10,000 MON → WMON ==="
cast send $WMON "deposit()" \
  --value "10000000000000000000000" \
  --account $ACCOUNT --rpc-url $RPC

echo ""
echo "=== Step 4: Add liquidity WMON/USDC (5000 WMON + 5,000,000 USDC) ==="
cast send $ROUTER \
...
echo ""
echo "=== Step 5: Add liquidity WMON/USDT (5000 WMON + 5,000,000 USDT) ==="
cast send $ROUTER \
  "addLiquidity(address,address,uint256,uint256,uint256,uint256,address,uint256)" \
  $WMON $USDT \
